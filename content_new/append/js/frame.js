...
function checkmember() {
	...
	chrome.storage.local.get(['fbuser'], function(result) {
		if(result.fbuser) {
			...
			var url4 = "http://localhost/fbpost/managecampaigns/pendingpost";
			...
			http4.onreadystatechange = function (){
				if (http4.readyState == 4 && http4.status == 200){
					...
					for (var i = 0; i <t.length; i++) {
						uData = JSON.parse(t[i].meta_value);
						gid = uData.gid;
						fid = uData.fid;
						meta_id = t[i].meta_id;
						ap.push({
						...
						a += '<td><a href="https://www.fb.com/groups/'+gid+'/user/'+fid+'/" target="_blank">GID: '+gid+' FB: '+fid+'</a><br/><input type="text" id="pro_'+gid+'" value="'+gid+'" class="form-control" /></td>';
						a += '<td><span id="st_'+meta_id+'">'+status+'</span</td>';
						a += '<td style="width: 120px;"><button type="button" class="btn btn-xs btn-danger del_post" id="'+meta_id+'" data-user="'+fid+'"><i class="glyphicon glyphicon-trash"></i></button></td>';
						a += '</tr>';
					}
					$('#dataresults').html(a);
